...
describe('UniswapV2Pair', () => {
  ...
  beforeEach(async () => {
	...
	const TestToken =  await ethers.getContractFactory("TestUniswapV2ERC20");
	const cPair = await ethers.getContractFactory("UniswapV2Pair")

	const tokenA = await TestToken.deploy(ethers.utils.parseEther('10000'))
	const tokenB = await TestToken.deploy(ethers.utils.parseEther('10000'))

	factory = await cFactory.deploy(wallet.address)

	await factory.createPair(tokenA.address, tokenB.address)
	const pairAddress = await factory.getPair(tokenA.address, tokenB.address)
	pair = await cPair.attach(pairAddress)
  ...
  it('feeTo:on', async () => {
    ...
    await pair.transfer(pair.address, expectedLiquidity.sub(MINIMUM_LIQUIDITY))
    await pair.burn(wallet.address)
    expect(await pair.totalSupply()).to.eq(MINIMUM_LIQUIDITY.add('332750694222842'))
    expect(await pair.balanceOf(other.address)).to.eq('332750694222842')
